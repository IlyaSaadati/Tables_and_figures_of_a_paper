# -*- coding: utf-8 -*-
"""
Created on Thu Aug 24 05:18:06 2023
...
#plot 1-1
box_plot=sns.boxplot(x='Exersice type 48', y='Excess weight loss 48 (%)', data=df,showcaps=False,palette='GnBu')
sns.stripplot(x='Exersice type 48', y='Excess weight loss 48 (%)', data=df, color='black', alpha=0.1,zorder=0)
plt.xlabel('Exercise type')
plt.ylabel('Loss of Excess Body Weight (%)')

# adding texts to boxes which represent number of data in each box
counts = df['Exersice type 48'].value_counts()
plt.text(-0.07, 132, f'N={counts[1]}', fontsize=8)
plt.text(0.9, 140, f'N={counts[2]}', fontsize=8)
plt.xticks([0, 1], ['aerobic', 'aerobic & strength'])
...
plt.savefig(r"F:\Safavi's project\figure_4_4_a.pdf",dpi=300,format="pdf")
...
dfac = df[df['Exersice type 48']==2]
dflow = df[df['Exersice type 48']==1]
...
axs[1].set_title('aerobic & strength')
axs[1].set_xlabel('')
axs[1].set_ylabel('')
...
# Adjust layout
plt.tight_layout()
plt.savefig(r"F:\Safavi's project\figure_4_4_b.pdf",dpi=300,format="pdf")
...
# creating subdata we need to work on
dfhac = df[df['Exersice type 48']==2]
dfhlow = df[df['Exersice type 48']==1]
...
# Adjust layout
plt.tight_layout()
plt.savefig(r"F:\Safavi's project\figure_4_4_c.pdf",dpi=300,format="pdf")
