# -*- coding: utf-8 -*-
"""
Created on Sun Sep  3 13:35:24 2023
...
df_in = DF[(DF['BMI 48 (kg/m2)']/DF['BMI Pre (kg/m2)'])>0.75]
df_su =DF[(DF['BMI 48 (kg/m2)']/DF['BMI Pre (kg/m2)'])<=0.75]
...
dataframe.append({'Insufficient Response':f'{counti1[1]}', 'Good Response':f'{counts1[1]}','P-value':f'{stats.ttest_ind(df_in["Exersice type Pre"],df_su["Exersice type Pre"]).pvalue}','Mean Difference (95%CI)':''})
dataframe.append({'Insufficient Response':f'{counti1[2]}', 'Good Response':f'{counts1[2]}','P-value':'','Mean Difference (95%CI)':''})
    
dataframe.append({'Insufficient Response':f'{counti2[1]}', 'Good Response':f'{counts2[1]}','P-value':f'{stats.ttest_ind(df_in["Exersice type 12"],df_su["Exersice type 12"]).pvalue}','Mean Difference (95%CI)':''})
dataframe.append({'Insufficient Response':f'{counti2[2]}', 'Good Response':f'{counts2[2]}','P-value':'','Mean Difference (95%CI)':''})
    
dataframe.append({'Insufficient Response':f'{counti3[1]}', 'Good Response':f'{counts3[1]}','P-value':f'{stats.ttest_ind(df_in["Exersice type 48"],df_su["Exersice type 48"]).pvalue}','Mean Difference (95%CI)':''})
dataframe.append({'Insufficient Response':f'{counti3[2]}', 'Good Response':f'{counts3[2]}','P-value':'','Mean Difference (95%CI)':''})

    
DataFrame=pd.DataFrame(dataframe)
DataFrame.index =index_list
print(DataFrame.iloc[1,:])
DataFrame.to_excel(r"J:\Safavi's project\table_4.xlsx", index=True   )
